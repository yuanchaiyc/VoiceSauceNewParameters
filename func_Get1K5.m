function [H1K5, F1K5, isComplete] = func_Get1K5(y, Fs, F0, handles, variables, textgridfile)
...
H1K5 = zeros(length(F0), 1) * NaN;
F1K5 = zeros(length(F0), 1) * NaN;
...
if (nargin == 5)
    ...
    for k=1:length(F0)
        ...
        [h1k5, f1k5] = func_GetHarmonics(yseg, 1500, F0_curr, Fs);
...
%--------------------------------------------------------------------------
function [h,fh]=func_GetHarmonics(data,f1_est,f0_est,Fs)
...
f_min = f1_est - df_range;
f_max = f1_est + df_range;

f = @(x)func_EstMaxVal(x, data, Fs);
options = optimset('Display', 'off');
warning off;
%[x, val, exitflag, output] = fmincon(f, f_est, [], [], [], [], f_min, f_max, [], options);
[x, val, exitflag, output] = fminsearchbnd(f, f1_est, f_min, f_max, options);  
