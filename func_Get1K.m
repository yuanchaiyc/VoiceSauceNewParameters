function [H1K, F1K, isComplete] = func_Get1K(y, Fs, F0, handles, variables, textgridfile)
...
H1K = zeros(length(F0), 1) * NaN;
F1K = zeros(length(F0), 1) * NaN;
...
if (nargin == 5)
    ...
    for k=1:length(F0)
        ...
        [h1k, f1k] = func_GetHarmonics(yseg, 1000, F0_curr, Fs);
