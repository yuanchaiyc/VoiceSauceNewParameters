function [H3K, F3K, isComplete] = func_Get3K(y, Fs, F0, handles, variables, textgridfile)
...
H3K = zeros(length(F0), 1) * NaN;
F3K = zeros(length(F0), 1) * NaN;
...
if (nargin == 5)
    ...
    for k=1:length(F0)
        ...
        [h3k, f3k] = func_GetHarmonics(yseg, 3000, F0_curr, Fs);
...
%--------------------------------------------------------------------------
function [h,fh]=func_GetHarmonics(data,f3_est,f0_est,Fs)
...
df = 0.5;     % search around f3_est in steps of df (in Hz)
df_range = round(f0_est*df); % search range (in Hz)

f_min = f3_est - df_range;
f_max = f3_est + df_range;

f = @(x)func_EstMaxVal(x, data, Fs);
options = optimset('Display', 'off');
warning off;
%[x, val, exitflag, output] = fmincon(f, f_est, [], [], [], [], f_min, f_max, [], options);
[x, val, exitflag, output] = fminsearchbnd(f, f3_est, f_min, f_max, options);  
