...
for k=1:length(paramlist)
    if (strcmp(paramlist{k}, 'H1*-A1*, H1*-A2*, H1*-A3*'))
        ...
        new_param_vec(func_getparameterlist('H1, H2, H3, H4')) = new_param_vec(func_getparameterlist('H1, H2, H3, H4')) + 2;
    ...
    elseif (strcmp(paramlist{k}, 'H1, H2, H3, H4'))
        new_param_vec(func_getparameterlist('H1, H2, H3, H4')) = 1;
        new_param_vec(func_getparameterlist(F0algorithm)) = new_param_vec(func_getparameterlist(F0algorithm)) + 2;

    elseif (strcmp(paramlist{k}, '1K'))
        new_param_vec(func_getparameterlist('1K')) = 1;
        new_param_vec(func_getparameterlist(F0algorithm)) = new_param_vec(func_getparameterlist(F0algorithm)) + 2;

    elseif (strcmp(paramlist{k}, '1K5'))
        new_param_vec(func_getparameterlist('1K5')) = 1;
        new_param_vec(func_getparameterlist(F0algorithm)) = new_param_vec(func_getparameterlist(F0algorithm)) + 2;
    ...
    elseif (strcmp(paramlist{k}, '3K'))
        new_param_vec(func_getparameterlist('3K')) = 1;
        new_param_vec(func_getparameterlist(F0algorithm)) = new_param_vec(func_getparameterlist(F0algorithm)) + 2;

    elseif (strcmp(paramlist{k}, '4K'))
        new_param_vec(func_getparameterlist('4K')) = 1;
        new_param_vec(func_getparameterlist(F0algorithm)) = new_param_vec(func_getparameterlist(F0algorithm)) + 2;
...
if (exist(matfile, 'file'))
    ...
    if (mod(new_param_vec(func_getparameterlist('F0 (Other)')), 2) == 0 && isfield(matdata, 'oF0'))
        if (length(matdata.oF0) == data_len)
    ...
    if (mod(new_param_vec(func_getparameterlist('Formants (Praat)')), 2) == 0 && isfield(matdata, 'pF1') && isfield(matdata, 'pF5'))
        if (length(matdata.pF1) == data_len)
    ...
    if (mod(new_param_vec(func_getparameterlist('Formants (Other)')), 2) == 0 && isfield(matdata, 'oF1'))
        if (length(matdata.oF1) == data_len)
    ...
    if (mod(new_param_vec(func_getparameterlist('H1, H2, H3, H4')), 2) == 0 && isfield(matdata, 'H1'))
        if (length(matdata.H1) == data_len)
            new_param_vec(func_getparameterlist('H1, H2, H3, H4')) = 0;
        end
    end

    if (mod(new_param_vec(func_getparameterlist('1K')), 2) == 0 && isfield(matdata, 'H1K'))
        if (length(matdata.H1K) == data_len)
            new_param_vec(func_getparameterlist('1K')) = 0;
        end
    end    

    if (mod(new_param_vec(func_getparameterlist('1K5')), 2) == 0 && isfield(matdata, 'H1K5'))
        if (length(matdata.H1K5) == data_len)
            new_param_vec(func_getparameterlist('1K5')) = 0;
        end
    end    
    ...
    if (mod(new_param_vec(func_getparameterlist('3K')), 2) == 0 && isfield(matdata, 'H3K'))
        if (length(matdata.H3K) == data_len)
            new_param_vec(func_getparameterlist('3K')) = 0;
        end
    end

    if (mod(new_param_vec(func_getparameterlist('4K')), 2) == 0 && isfield(matdata, 'H4K'))
        if (length(matdata.H4K) == data_len)
            new_param_vec(func_getparameterlist('4K')) = 0;
        end
    end    
...
if (new_param_vec(func_getparameterlist('H1, H2, H3, H4')) == 1)
    new_paramlist{cnt} = 'H1, H2, H3, H4';
    cnt = cnt + 1;
end

if (new_param_vec(func_getparameterlist('1K')) == 1)
    new_paramlist{cnt} = '1K';
    cnt = cnt + 1;
end

if (new_param_vec(func_getparameterlist('1K5')) == 1)
    new_paramlist{cnt} = '1K5';
    cnt = cnt + 1;
end
...
if (new_param_vec(func_getparameterlist('3K')) == 1)
    new_paramlist{cnt} = '3K';
    cnt = cnt + 1;
end

if (new_param_vec(func_getparameterlist('4K')) == 1)
    new_paramlist{cnt} = '4K';
    cnt = cnt + 1;
end
