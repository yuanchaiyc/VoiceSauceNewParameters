function [H4K, F4K, isComplete] = func_Get4K(y, Fs, F0, handles, variables, textgridfile)
...
H4K = zeros(length(F0), 1) * NaN;
F4K = zeros(length(F0), 1) * NaN;
...
if (nargin == 5)
    ...
    for k=1:length(F0)
        ...
        [h4k, f4k] = func_GetHarmonics(yseg, 4000, F0_curr, Fs);
